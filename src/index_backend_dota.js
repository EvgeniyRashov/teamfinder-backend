...
const UserSchema = new mongoose.Schema({
  ...
  activeGame:      { type: String, default: 'CS2' }, // CS2, Dota2
  dotaMmr:         { type: Number, default: 0 },
  dotaRole:        { type: String, default: 'any' },
...
app.post('/api/settings', authMiddleware, async (req, res) => {
  try {
    const { nick, bio, role, mode, region, lang, elo, faceit, mmrank, hasMic, activeGame, dotaMmr, dotaRole } = req.body;
    const upd = {};
    ...
    if (activeGame !== undefined) upd.activeGame = activeGame;
    if (dotaMmr !== undefined) upd.dotaMmr = Number(dotaMmr) || 0;
    if (dotaRole !== undefined) upd.dotaRole = dotaRole;
...
app.get('/api/players', async (req, res) => {
  try {
    const { game, role, dotaRole, region, language, minTrust, minElo, minFaceit, minDotaMmr, mmrank, hasMic, limit = 50 } = req.query;
    const filter = { steamid: { $nin: ['undefined', 'null'], $exists: true }, isBanned: { $ne: true } };

    if (game && game !== 'any') filter.activeGame = game;
    if (role && role !== 'any') filter.role = role;
    if (dotaRole && dotaRole !== 'any') filter.dotaRole = dotaRole;
    if (minDotaMmr) filter.dotaMmr = { $gte: Number(minDotaMmr) };
    ...
    const players = await User.find(filter)
      ...
      .limit(Number(limit))
      .select('steamid name avatar nick bio elo faceit mmrank role mode region language trustScore hasMic isOnline isLookingForTeam lastSeen isAdmin activeGame dotaMmr dotaRole');
    res.json(players);
  } catch(err) {
  ...
  try {
    ...
    if (queue.length < 2) {
        for (const p1 of queue) {
            ...
            if (waitSec > 45) {
                await Lobby.create({ 
                    ownerId: p1.steamid, members: [p1.steamid], game: p1.game, gameMode: p1.mode, autoFill: true, status: 'waiting'
                });
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      for (let j = i + 1; j < queue.length; j++) {
        ...
        const p2 = queue[j];
        if (p1.game !== p2.game) continue;
        if (p1.game === 'CS2' && p1.mode !== p2.mode) continue;
        let match = false;
        if (p1.game === 'Dota2') {
           if (Math.abs(p1.dotaMmr - p2.dotaMmr) <= (eloRange)) match = true;
        } else if (p1.mode === 'FACEIT' && p1.faceit > 0 && p2.faceit > 0) {
      ...
      if (group.length > 1) {
        ...
        await Lobby.create({ 
            ownerId: members[0], members, game: p1.game, gameMode: p1.mode, autoFill: !isFull, status: isFull ? 'matched' : 'waiting'
        });
...
app.post('/api/matchmaking/start', authMiddleware, async (req, res) => {
  try {
    ...
    const game = req.body.game || me?.activeGame || 'CS2';
...
app.post('/api/matchmaking/autofill', authMiddleware, async (req, res) => {
  try {
    ...
    const me = await User.findOne({ steamid: req.user.steamid });
    const game = lobby.game || me?.activeGame || 'CS2';
    ...
    for (const c of candidates) {
      let eloOk = true;
      if(game === 'Dota2') {
         eloOk = !me?.dotaMmr || !c.dotaMmr || Math.abs((me.dotaMmr || 0) - c.dotaMmr) <= 1500;
      } else {
         eloOk = !me?.elo || !c.elo || Math.abs((me.elo || 0) - c.elo) <= 900;
      }
...
app.get('/api/profile/:steamid', async (req, res) => {
  ...
  try {
    const [summaryRes, statsRes, hoursRes] = await Promise.allSettled([
      ...
      fetch(`https://api.steampowered.com/IPlayerService/GetOwnedGames/v1/?key=${key}&steamid=${steamid}&appids_filter[0]=730&appids_filter[1]=570&include_appinfo=false`)
    ]);
    ...
    if (hoursRes.status === 'fulfilled' && hoursRes.value.ok) {
      ...
      const csGame = games.find(g => String(g.appid) === '730');
      const dotaGame = games.find(g => String(g.appid) === '570');
      if (csGame) hoursCs2 = Math.round(csGame.playtime_forever / 60);
      if (dotaGame) hoursDota2 = Math.round(dotaGame.playtime_forever / 60);
    }

    const dbUser = await User.findOne({ steamid }).select('elo faceit mmrank role mode region nick bio trustScore friends commends isOnline hasMic isAdmin isBanned activeGame dotaMmr dotaRole');
    const reports = await Report.find({ targetSteamId: steamid }).sort({ createdAt: -1 }).limit(10);

    return res.json({ profile, stats, hoursCs2, hoursDota2, gameData: dbUser || null, friends: dbUser?.friends || [], reports });
  } catch(err) {
